...
(deftest backup-script-with-service
  (testing
    ...
    (is (= ["#!/bin/bash"
            ...
            "tar cvzf /var/backups/transport-outgoing/service-name_letsencrypt_file_${timestamp}.tgz /etc/letsencrypt/accounts/ /etc/letsencrypt/csr/ /etc/letsencrypt/keys/ /etc/letsencrypt/renewal/"
            "chown dda-backup:dda-backup /var/backups/transport-outgoing/service-name_letsencrypt_file_${timestamp}.tgz"
            ""
            "#backup the specified files and directories"
            "tar cvzf /var/backups/transport-outgoing/service-name_liferay_file_${timestamp}.tgz /var/lib/liferay/data/document_library/ /var/lib/liferay/data/document_library/images/"
