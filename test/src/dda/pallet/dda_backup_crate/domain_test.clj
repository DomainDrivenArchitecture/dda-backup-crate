...
(def test-domain-backup-config
  {:backup-name "test"
   ...
   :transport-management {:ssh-pull true
                          :duplicity-push {:public-key gpg-public
                                           :private-key gpg-private
                                           :passphrase gpg-passphrase
                                           :root-password {:clear-password "root-pw"}}}
   :backup-elements [test-backup-element1]})

(def user-domain-config
  {:root {:clear-password "root-pw"
          :gpg {:trusted-key {:public-key  gpg-public
                              :private-key gpg-private
                              :passphrase  gpg-passphrase}}}
